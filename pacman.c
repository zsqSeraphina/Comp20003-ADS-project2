...
//data get when getting next move, to print in output.txt
...
void ExitProgram(const char *message) {
    ...
    double time_spent = (double)(end - begin) / CLOCKS_PER_SEC; //to calculate time spent
    endwin();                       //Uninitialize ncurses and destroy windows
    delscreen(mainScreen);
    printf("%s\n", message);        //Display message
    printf("Final Score: %d\n",Points);
    char prop[4];
    //turn the enum propagation to string
    if(propagation == max){strcpy(prop, "MAX");}
    if(propagation == avg){strcpy(prop, "AVG");}
    ...
    fprintf(output, "Propagation=%s\n"
            ...
            "Expanded/Second=%f\n"
            "Score=%d\n", 
            prop, budget, max_depth, 
            generated, expanded, time_spent, 
            expanded/time_spent, Points);
...
void MainLoop() {
    ...
    do {
        ...
        if(ai_run){
            ...
            generated += get_generated_nodes();
            expanded += get_expanded_nodes();
            unsigned depth = get_max_depth();
            if(max_depth < depth){
                max_depth = depth;
            }
